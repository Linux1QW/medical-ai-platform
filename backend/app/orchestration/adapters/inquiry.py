...
class InquiryAdapter(BaseAgentAdapter):
    agent_name = "inquiry"

    async def _call_agent(self, context: EvaluationContext) -> dict:
        patient_info = self._build_patient_info(context)
        return await run_inquiry_analysis(context.conversation_text, patient_info)
    ...
    def _parse_result(self, raw: dict) -> AgentResultEnvelope:
        ...
        try:
            ...
            logger.warning("inquiry_agent raw_response JSON 解析失败，使用默认值")
            data = {}
