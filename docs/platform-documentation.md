...
本文面向医生用户、教师/管理员和一线运维人员。完整接口、配置、架构、部署和限制以 [PROJECT_GUIDE](PROJECT_GUIDE.md) 为准。事实基线为当前仓库的 V1.2 功能代码；生产验收状态只以 `release-evidence/v1.2/final-acceptance.json` 为准。
...
`AdminReviews` 已接入 `/admin/reviews`，管理员可在前端处理人工复核。知识库、模型版本和监控仍没有覆盖全部管理操作，缺少的操作请使用 <http://localhost:8000/docs>。模型版本“rollback”只改登记状态，不回滚 RAG generation。
